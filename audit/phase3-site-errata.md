...
| 51 | `docs/c-api.html#status`, `docs/verification.html#coverage-reports` | release gate: "Not ready: 1 unmet criterion" / "9 of 10 criteria met" (from the committed `RELEASE_GATE.md`) | measured at TARGET by running the gate: 2 criteria unmet and `--check` fails (B9, executed in the read-only TARGET tree) | B9 | open |
| 52 | `development/internals/bindings/c-api-internals.html#evidence-gaps`, `development/handbook/update-the-c-api.html#traps` | child-first destruction is "the caller's obligation" | `graphics_device.h` allows destroying the device first (the device disposes its resources) | B9 | open |
| 53 | `development/internals/modules/devices-ext.html#test-seams` | a real platform's dialog callback does not fire inside the call | on native WIN32 the file dialogs block and call back before `Show*` returns (`Win32SystemServices.cpp`) — B8 NEW-B8-22 | B8 | open |
| 54 | `development/internals/modules/devices-ext.html#dialogs` | no test shows asynchronous timing | the X11 and Wayland portal tests do, at platform level | B8 | open |
| 55 | `development/internals/modules/devices-ext.html#change` | "runs the ten CNA::Devices suites" | only eight match the devices workflow filter (see CNA-BUG-035) | B8 | open |
