...
- The oracle corpus references were captured under Wine + DXVK on Linux. The repository records all 39 scenes as pixel-exact on the DIRECTX9 oracle path (README status, per-scene notes, and a CTest that iterates all 39 at zero tolerance); the last consolidated dated divergence report covers 31 of them and later per-scene notes record the rest. No committed dated log of a 39-scene run exists, no 39-scene run was executed for this documentation, and none of it is in CI. The site words it exactly that way (verification page has a "Where the 39 / 39 comes from" callout).
