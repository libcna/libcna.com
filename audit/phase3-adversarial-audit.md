...
| AF-018 | Known Issues text | Hand-written references from one entry to another by final id went wrong when a later reclassification shifted the id allocation: CNA-BUG-268 cited CNA-GAP-068 (the video gap) for the network-session gap; a reviewer's text cited a reclassified entry by number. | **fixed**: `{{issue:KEY}}` placeholders resolved after allocation (the 3 references inside entry text); the 11 references on public pages were first only read against the entry they name, and are now guarded (AF-024) |
...
| AF-024 | Known Issues references on public pages | The 14 references AF-018 read once (11 page-and-entry pairs on 10 hand-written pages, 12 occurrences because `deep-dives/services/network-sessions.html` cites CNA-BUG-267 twice, once as an unlinked mention; plus 3 inside entry text) named entries by ids that the audit allocated after all stable ones and that shift if a later reclassification changes the allocation order. A reading is a snapshot: after a shift every one of them would have pointed at a different defect and no validator would have failed. | **fixed**: each of the 12 hand-written occurrences is written `<!--issue:KEY--><a href=…>CNA-…</a><!--/issue-->` (KEY is `NEW-AUDIT-nn`, or the earlier id of a reclassified entry); `scripts/issue_refs.py apply` rewrites the marker from `dispositions.json` and `check`, part of `validate_all.sh`, fails on a stale marker, an unresolvable key, or a link to or mention of such an entry outside a marker. The reviewed-duplicate pairs (`duplicate-review.json`) take the same keys. **Proved by a temporary shift** (the reclassifications of CNA-BUG-079 and CNA-GAP-050 withdrawn, everything rebuilt): before `apply`, `check` failed and named the 10 stale pages; afterwards all 12 markers had moved to a new number and named the entry with the identical title, the 3 generated references and the 121 generated backlink blocks followed (none pointed at another entry), and the whole validation passed. The first run of that experiment failed in one more place, the duplicate review (a pair named by allocated ids), which is why it now takes keys; the experiment was reverted afterwards |
