...
* **Reference sources on this host (read-only)** for settling what the XNA contract really is, in addition to TARGET: `~/deps/xna-il-cache/*.il` (IL disassembly of the genuine XNA 4.0 assemblies — decisive for "does XNA do this?"),
  `~/deps/xna40-windows-assemblies`, `/rv/data/development/github.com/libcna/xna4-decomp`, `/rv/data/development/github.com/libcna/xna4-spec` (Microsoft's API documentation), `~/deps/FNA3D`, and the sibling repositories under
  `/rv/data/development/github.com/libcna/` (sharp-runtime `next`, easy-gl, meta-gl, cna-samples …; state the revision when you rely on one). A claim about XNA behaviour should cite which of these decided it.
* **Syntax-check flags that work** for CNA headers: `g++ -std=c++23 -fsyntax-only -DCNA_RENDERER_SDL_RENDERER -DCNA_PLATFORM_SDL3 -I<target>/modules/<m>/include … -I<sharp-runtime next>/include`; some headers need a renderer define (`-DCNA_RENDERER_EASYGL`). Name the flags in the record.
* **Ledger tokens** are matched against the destination page's article text (`<main>` text for the protected root pages that have no `<article>`), whitespace-normalised, case-sensitive; inline tags add no space, so a token may span `<code>…</code>`.
* **Hub pages** (`deep-dives/index.html`, `deep-dives/<group>/index.html`) are generated by the orchestrator; a missing hub is not an error for you.
* **Deep-dive references to Known Issues:** do not hard-code `CNA-BUG-###` links to pages that may not exist; mention the id in plain text ("see Known Issues") and the orchestrator adds the backlinks.

## 10. Lean rules for the remaining packages (binding; usage budget)

The pilot packages used ~450–780 K tokens each. To finish within budget, every package launched from now on follows these rules; correctness and TARGET verification are NOT relaxed, only waste is:

1. **No helper sub-agents.** Do the reading yourself (they were the largest hidden cost and their results were lost when a run was stopped).
2. **Batch tool calls.** Issue independent reads/greps in one message; read a file once and keep what you need; never re-read the guides or a large source file; prefer `sed -n`/`rg` on the part you need over whole-file reads of big files.
3. **Concept density: 15–25 concepts per 1,000 Bible words** (not 40). One concept per distinct rule/semantic/caveat/example/evidence statement; do not split a sentence into three concepts; every heading still needs a concept or an `empty_sections` entry. Depth of the *pages* is unchanged: exact semantics, edge cases, evidence, source links.
4. **Pages: 2–4 pages per package**, 1,800–3,500 words each; no separate tiny pages.
5. **Persist once early, finalise once**: write the record JSON in as few large writes as practical (one per unit), build pages once, fix errors, finish. Do not re-run the whole ledger check repeatedly — run it for your units at the end and after fixes.
6. **Report ≤ 250 words.**
