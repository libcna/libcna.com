...
## 8. Packages B10–B12 (candidates recorded by the chapter and auxiliary packages)

Your candidates (`audit/data/bible/issues/candidates-B10|B11|B12.json`) were noticed by chapter packages while absorbing Bible chapters; each carries the author's `text`, `kind`, `target_reading` and `evidence` (a TARGET path/symbol) — already pre-filtered (items read as absent, "not-an-issue" and ones already covered by the Bible-ledger verification were dropped). Verify each one **from its cited evidence**: re-read the cited function and its callers/tests at TARGET, decide the classification (§2), and publish per §3. You do not need to re-derive the whole subsystem. Before publishing, check `audit/data/bible/issues/verified-B1..B9.json` for the same defect (grep by symbol): if it is already published there, disposition yours as STILL EXISTS pointing at your own entry and say in `note` that it duplicates <id> so the orchestrator can merge. Models: `verified-B2.json`, `verified-B8.json`. A candidate that reads as a documented boundary is a functional gap / platform limitation, not a bug; one whose only evidence is "no test" is a verification gap. Keep entries concise but complete (the §3 fields); do not pad. Report ≤250 words.
