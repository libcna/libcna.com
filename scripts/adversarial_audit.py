...
APPLIED = ROOT / "audit" / "data" / "adversarial" / "applied-page-fixes.json"
FINDING_OUTCOMES = ROOT / "audit" / "data" / "adversarial" / "unit-finding-outcomes.json"   # {"C03-ch12-F4": {"outcome": "rejected|restored|no-change", "note": "..."}} for findings without a page fix
...
def counts_block() -> str:
    ...
    bc = c["by_class"]
    applied = {a["id"] for a in json.loads(APPLIED.read_text(encoding="utf-8")).get("applied", [])} if APPLIED.exists() else set()
    manual = json.loads(FINDING_OUTCOMES.read_text(encoding="utf-8")) if FINDING_OUTCOMES.exists() else {}
        ...
        for f in sorted(ur_dir.glob("*.json")):
            ...
            for u in json.loads(f.read_text(encoding="utf-8")).get("units", []):
                ...
                for fd in u.get("findings", []):
                    fid = fd["id"]
                    if fid in manual:
                        out = manual[fid]["outcome"]
                    elif any(a == f"{grp}-{fid}" or a.startswith(f"{grp}-{fid}-") for a in applied):
                        out = "fixed"
                    else:
                        out = "open"
                    outcome[out] += 1
                    kinds[fd["type"]] += 1
                d, co = u.get("dropped") or {}, u.get("corrections") or {}
                unit_rows.append(f"| `{u['unit']}` | {u['status']} | {'yes' if u.get('large') else ''} | " + (", ".join(f"{k} {v}" for k, v in sorted(kinds.items())) or "none")
                                 + f" | {d.get('reviewed', 0)} / {d.get('false_drop', 0)} | {co.get('verified', 0)} / {co.get('agree', 0)} |")
    lines = [COUNTS_BEGIN, "",
             ...
             "| Unit | Status | Large | Findings | Dropped | Corrections |", "|---|---|---|---|---:|---:|", *unit_rows, "",
             COUNTS_END]
    return "\n".join(lines)
